...
class SpoonacularFacade(object):
    ...
    def format_recipe_data_as_html(cls, recipe_data):
        ...
        """
        ingredients = "\n".join([
            strip_tags(ingredient["original"]) for ingredient
    ...
    def gen_recipe_card(self, id):
        """Creates recipe card from the API given a set of ids.
        Args:
            ids: list of one or more Spoonacular recipe ids.
        Returns:
            A list of dictionaries (json-like) containing the data for
            each recipe.
        """
        #response = self.client.get_recipe_information_bulk(id)
        response = self.client.create_recipe_card(id)
        self.check_status_and_raise(response)
        recipes = response.json()
        return recipes